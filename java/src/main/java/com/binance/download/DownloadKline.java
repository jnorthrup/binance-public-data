...
public class DownloadKline {

    public static void downloadMonthlyKlines(Enums.TRADING_TYPE tradingType, List<String> symbols, int numSymbols, List<Enums.INTERVALS> intervals, List<Enums.YEARS> years, List<Enums.MONTHS> months, String startDate, String endDate, String folder, boolean checksum) throws IOException {
        int current = 0;
        ...
        for (String symbol : symbols) {
            System.out.println("[" + (current + 1) + "/" + numSymbols + "] - start download monthly " + symbol + " klines ");
                ...
                for (Enums.YEARS year : years) {
                    for (Enums.MONTHS month : months) {
                        ...
                        if (!currentDate.isBefore(start) && !currentDate.isAfter(end)) {
                            String path = Utility.getPath(tradingType, "klines", "monthly", symbol);
                            ...
                            if (checksum) {
                                String checksumPath = Utility.getPath(tradingType, "klines", "monthly", symbol);
    ...
    public static void downloadDailyKlines(Enums.TRADING_TYPE tradingType, List<String> symbols, int numSymbols, List<Enums.INTERVALS> intervals, List<LocalDate> dates, String startDate, String endDate, String folder, boolean checksum) throws IOException {
        int current = 0;
        String dateRange = null;
        ...
        for (String symbol : symbols) {
            System.out.println("[" + (current + 1) + "/" + numSymbols + "] - start download daily " + symbol + " klines ");
            for (Enums.INTERVALS interval : intervals) {
                for (LocalDate date : dates) {
                    if (!date.isBefore(start) && !date.isAfter(end)) {
                        String path = Utility.getPath(tradingType, "klines", "daily", symbol);
                        String fileName = symbol.toUpperCase() + "-" + interval + "-" + date + ".zip";
                        Utility.downloadFile(path, fileName, dateRange, folder);

                        if (checksum) {
                            String checksumPath = Utility.getPath(tradingType, "klines", "daily", symbol);
